...
export default function StudentElections() {
  ...
  const statusBadge = (status) => {
    ...
    if (status === "Upcoming")
      return (
        <span className="text-xs font-semibold text-yellow-700 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/30 px-2.5 py-1 rounded-full">
          Upcoming
        </span>
      );
    return (
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Participate in your institution's democratic process</p>
      </div>
      ...
      {elections.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center border border-gray-200 dark:border-slate-700">
          ...
          <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">No Elections Yet</h3>
          <p className="text-gray-500 dark:text-gray-400 text-sm">Elections created by admin will appear here.</p>
        </div>
      ) : (
        <>
          {live.length > 0 && (
          ...
          {upcoming.length > 0 && (
            <section>
              <h2 className="text-sm font-bold uppercase tracking-wider text-yellow-600 dark:text-yellow-400 mb-3">
