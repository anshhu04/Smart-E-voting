...
export default function StudentDashboard() {
  ...
  // Read profile photo saved by StudentProfile
  ...
  const pendingVotes = liveElections.filter(
    (e) => !localStorage.getItem("vote_" + user.studentId + "_" + e.id)
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M5.636 18.364a9 9 0 010-12.728m12.728 0a9 9 0 010 12.728M12 12a1 1 0 100-2 1 1 0 000 2z" />
        </svg>
      ),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <div
            key={i}
            className={`bg-white dark:bg-slate-900 rounded-2xl p-5 border ${s.border} shadow-sm`}
          >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
          ...
          {liveElections.length === 0 ? (
            ...
            <div className="space-y-3">
              {liveElections.map((e) => {
                const hasVoted = localStorage.getItem("vote_" + user.studentId + "_" + e.id);
                return (
                  <div key={e.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-slate-800">
                    ...
                    {hasVoted ? (
                      <span className="ml-3 flex-shrink-0 text-xs font-semibold text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-2.5 py-1 rounded-full">
                        ✓ Voted
                      </span>
                    ) : (
                      <button
                        onClick={() => navigate("/student/vote/" + e.id)}
                        className="ml-3 flex-shrink-0 text-xs font-semibold bg-blue-700 hover:bg-blue-800 text-white px-3 py-1.5 rounded-lg transition"
                      >
                        Vote Now
                      </button>
                    )}
          ...
          {votedElections.length === 0 ? (
            ...
            <div className="space-y-3">
              {votedElections.slice(0, 5).map((e) => {
                ...
                return (
                  <div key={e.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-slate-800">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-slate-700">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {elections.filter((e) => getStatus(e) === "Upcoming").map((e) => (
              <div key={e.id} className="flex items-center gap-3 p-3 rounded-xl bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800">
